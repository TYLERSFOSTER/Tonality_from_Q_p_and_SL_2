...
\KwData{{\bf (i)} positive integers $m,n,k\in\ZZ_{>0}$, {\bf (ii)} a set of $k$ distinct $m$-dimensional vectors $v_1,\dots,v_k\in\FF_{\!p}^m$ such that the first entry of each vector is nonzero, {\bf (iii)} a set of $k$ integers $e_1,\dots,e_k\in\ZZ$, {\bf (iv)} a primitive root $q$ modulo $p^n$, {{\bf (v)}} a primitive $p^{n-1}(p-1)^\text{st}$-root of unity $\zeta\in\CC^\times$.}
...
$q \gets$ {\bf primitive root modulo} $p^n$ \tcp*[r]{running error, to modify over course of algorithm}
$m\gets \text{max}\{m,n\}$;
\\ 
\vdots
\end{algorithm}
