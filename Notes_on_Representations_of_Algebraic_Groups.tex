    ...
    \tikz[x=1.2ex, y=1.2ex, baseline=-0.05ex]{% 
        \begin{scope}[x=1ex, y=1ex]
            ...
            \path[draw, 
                ...
                (0,0) rectangle (1,1);
        \end{scope}
        \path[draw, line width = 1.0] (0.5, 0.5) 
            -- (1, 1);
        \path[draw, line width = 1.0] (0.6, 1) 
            -- (1, 1) -- (1, 0.6);
        }
    }
...
\begin{comment}{Adele-ish products}
\ 
{\color{red} [...]}
...
\begin{subsubsection}
{\bf Tate's thesis and ``arithmetic Pontryagin duality."}
\normalfont
{\color{red} [...] }
\end{subsubsection}
...
{\color{red} [...]}
	...
	\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
	\begin{xy}
	(0,0)*+{\includegraphics[scale=.1]{3_adic_Berkovich.jpg}};
...
f(x)\ \!\omega(x)\ \!|x|^{s}_{p}\ \!d^\times\!x$ as a function of $s$.}
\KwData{{\bf (i)} positive integers $m,n,k\in\ZZ_{>0}$, {\bf (ii)} an ordered set $V=(v_1,\dots,v_k)$ of $k$ {\em distinct} vectors in $\FF_{\!p}^m$ such that the first entry of each vector is nonzero, {\bf (iii)} an ordered set $E=(e_1,\dots,e_k)$ of $k$ integers, {\bf (iv)} an ordered set $Z=(z_1,\dots,z_k)$ of $k$ complex numbers, {\bf (v)} a primitive root $q$ modulo $p^n$, {{\bf (vi)}} a $p^{n-1}(p-1)^\text{st}$-root of unity $\zeta\in\CC^\times$.\\ The idea here is that $v_i=(v_{i,0}, v_{i,2},\dots,v_{i,m-1})$ and $e_i$ together represent the compact open $U_i=p^{e_i}(v_{i,0}+v_{i,1}p+\cdots+v_{i,m-1}p^{m-1})+p^{m+e_{i}}\ZZ_{p}$, with $f(U_i)=z_i$, and that $\omega$ is uniquely determined by the condition $\omega(q)=\zeta$.}
\KwResult{A set of complex numbers $c_1,\cdots,c_\ell$ and positive real numbers $\lambda_1,\dots,\lambda_\ell$ such that the function $F(s)=c_1 e^{i2\pi\lambda_1s}+\cdots+c_\ell e^{i2\pi\lambda_\ell s}$ is the $p$-adic Mellin transform.}
$W\gets$ {\bf empty dictionary} $\{\}$\;
\For{$j$ {\bf in range} $1\le j\le p^{n-1}(p-1)$}{
$W[q^j]\gets\zeta^j$ \tcp*[r]{Log values of $\omega$ for all elements in $\ZZ_p^\times/(1+p^n\ZZ_p)\cong(\ZZ/p^n\ZZ)^\times$}
}
\If{$n>m$}{
$V'\gets$ {\bf empty\ ordered\ set} $()$\;
$E'\gets$ {\bf empty\ ordered\ set} $()$\;
$Z'\gets$ {\bf empty\ ordered\ set} $()$\;
\For{$v_i$ {\bf in} $V$}{
      \For{$u$ {\bf in} $\FF^{n-m}_{\!p}$}{
      $v\gets$ {\bf concatenated} $v_i|u\in\FF^{n}_{\!p}$\;
      $V'\gets V'+(v)$\;
      $E'\gets E'+(e_i)$\;
      $C'\gets Z'+(z_i)$\;
   }
   ...
   $E\gets E'$\;
   $Z\gets Z'$\;
   $m\gets n$ \tcp*[r]{$f$ now described at resolution finer than or equal to that of $\omega$}
   }
$C\gets$ {\bf empty\ ordered\ set} $()$\;
$\Lambda\gets$ {\bf empty\ ordered\ set} $()$\;
\For{$v_i=(v_{i,0},\ \!v_{i,1},\ \!\dots,\ \!v_{i,m-1})$ {\bf in} $V$}{
$x\gets v_{i,0}+v_{i,1}p+\cdots+v_{i,m-1}p^{m-1}$  \tcp*[r]{$x$ is an element of $(\ZZ/p^m\ZZ)^\times$}
$x\gets x\ (\text{mod}\ p^n)$  \tcp*[r]{pass to the image of $x$ is in $(\ZZ/p^n\ZZ)^\times$}
$\omega_i\gets W[x]$  \tcp*[r]{compute $\omega(x)$ via look-up in $W$}
$c_i\gets\frac{p^{m+1}}{p^{e_{i}}(p-1)}\ \omega_i$ \;
$\lambda_i\gets -e_i\ \text{log}(p)$\;
$C\gets C+(c_i)$\;
$\Lambda\gets \Lambda+(\lambda_i)$ \tcp*[r]{$C=(c_1,...,c_\ell)$ and $\Lambda=(\lambda_1,...,\lambda_\ell)$ with $\widetilde{f}(s,\omega)=c_1 e^{i2\pi\lambda_1s}+\cdots+c_\ell e^{i2\pi\lambda_\ell s}$\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!\!}
}
\end{algorithm}
...
\begin{remark}
{\bf Finding a primitive root modulo $\pmb{p^n}$ for use in Algorithm \ref{algorithm: Mellin}.}
{\color{red} [...]}
\end{remark}
...
Here $\varphi(5^3)=\#(\ZZ/5^3\ZZ)^\times$, the value of Euler's totient function at $5^3$. The significance of the element $2\in\ZZ_5$ in our definition of $\omega$ is that $2$ is a primitive root modulo $5^3$, which is to say that it is a generator of the multiplicative group $\ZZ^{\times}_5/(1+5^3\ZZ_5)\cong(\ZZ/5^3\ZZ)^\times$.
	...
	\begin{equation}\label{equation: real part of Mellin 00}
	F(t)\ :=\ \text{Re}\ \widetilde{f}(\omega,1\!+\!i2\pi t)
	\end{equation}
obtained from this $p$-adic Mellin transform.
	\begin{figure}[ht]
	\vskip -.2cm
	$$
	\includegraphics[scale=.325]{Mellin_00.jpg}
	$$
	\vskip -.4cm
	\caption{{\bf Real part of Mellin transform of $\pmb{f}$ and $\pmb{\omega}$ in Equations \eqref{equation: cslc f for Mellin 01} and \eqref{equation: norm un char for Mellin 00}.} {\color{red} [...]}}
	\label{figure: Mellin 00}
	\end{figure}
	
	Observe that the real-valued function defined in Equation \ref{equation: real part of Mellin 00} depends not only on the parameter $s=1+i2\pi t$, but also on the normalized unitary character $\omega$. There exist $\varphi(5^3)=(5-1)\!\cdot\! 5^2=100$ distinct normalized unitary characters on $\QQ^\times_5$ with conductor $3$. Each such character is uniquely determine by the power $\zeta$ to which it takes the element $2\in\ZZ_5$. Let us denote by $\omega_k$, for $1\le k\le 100$, the normalized unitary character satisfying
	$$
	\omega_k(2)\ =\ \zeta^k,
	\ \ \ \text{with cond}_5(\omega)=5^3.
	$$

	As demonstrated by Figure \ref{figure: char changes Mellin 00} below, variation of the parameter $\omega$ in Equation \eqref{equation: real part of Mellin 00} through these characters $\omega_k$ leads to timbral changes in resulting audio signal, while still keeping features of the signal's underlying tone intact.
	\begin{figure}[ht]
	\vskip -.3cm
	$$
	\begin{array}{rl}
	...
	\begin{xy}
	(0,0)*+{
	\includegraphics[scale=.25]{Mellin_001.jpg}
	};
	\end{xy}
	\\[-2pt]
	\pmb{\omega_2(2)=\zeta^2:}
	&
	\begin{xy}
	(0,0)*+{
	\includegraphics[scale=.25]{Mellin_002.jpg}
	};
	\end{xy}
	\\[-2pt]
	\pmb{\omega_3(2)=\zeta^3:}
	&
	\begin{xy}
	(0,0)*+{
	\includegraphics[scale=.25]{Mellin_003.jpg}
	};
	\end{xy}
	\\[-2pt]
	\pmb{\omega_4(2)=\zeta^4:}
	&
	\begin{xy}
	(0,0)*+{
	\includegraphics[scale=.25]{Mellin_004.jpg}
	};
	\end{xy}
	\end{array}
	$$
	\vskip -.5cm
	\caption{{\bf Timbral changes coming from variation of $\pmb{\omega}$ in Equation \eqref{equation: real part of Mellin 00}.} As we vary the normalized unitary character $\omega$ that appears in Equation \ref{equation: real part of Mellin 00}, the timbral character of the resulting audio signal changes, even though its underlying tone remains intact.}
	\label{figure: char changes Mellin 00}
	\end{figure}
...
\begin{subsection}{Harmonic movement from $\bold{GL}_1(\QQ_p)$-action on {\em p}-adic Mellin transforms}
\ {\color{red} [...]}

One important point here is that because $\QQ^\times\!\subset\QQ^\times_p$, every instance of just-intonal harmonic movement, in the standard sense, has a direct counterpart for ``$p$-adic" audio signals obtain via $p$-adic Mellin transform. Thus every $p$-adic audio signal $\widetilde{f}(\omega, s)$ can be translated along all the standard just intervals:
\vskip -.3cm
	$$
	\begin{array}{rrcl}
	\text{an octave ``up":}
	...
	\overset{
	\begin{tikzpicture}
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{1}{2}}(\omega, s),
	\\[6pt]
	\text{an octave ``down":}
	&
	\widetilde{f}(\omega,s)
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{2}{3}}(\omega, s),
	\\[6pt]
	\text{a just perfect fifth ``down":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{3}{2}}(\omega, s),
	\\[6pt]
	\text{a just perfect fourth ``up":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{3}{4}}(\omega, s),
	\\[6pt]
	\text{a just perfect fourth ``down":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{4}{3}}(\omega, s),
	\\[6pt]
	\text{a just major third ``up":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{4}{5}}(\omega, s),
	\\[6pt]
	\text{a just major third ``down":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{5}{4}}(\omega, s),
	\\[6pt]
	\text{a just minor third ``up":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
	...
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{5}{6}}(\omega, s),
	\\[6pt]
	\text{a just minor third ``down":}
	&
	\widetilde{f}(\omega,s)
	&
	\!\!\!\mapsto\!\!\!
...
\begin{example}
{\bf Translating a $3$-adio signal ``up" a just major third.}
\normalfont
To provide a concrete example, suppose $p=3$, and that we have an audio signal obtained as a $3$-adic Mellin transform
	\begin{equation}\label{equation: 3-adic Mellin pre-translate}
	\widetilde{f}(\omega,s)
	\ \ =\ 
	\underset{\QQ^\times_3}{\int}\!f(x)\ \omega(x)\ |x|^{s}_3\ d^\times\!x.
	\end{equation}
Let us be agnostic about the conductor of either $f$ or $\omega$. Imagine that we want to translate this $3$-adic audio signal ``up" by a just major third. This means that we want to replace the $3$-adic Mellin transform in Equation \eqref{equation: 3-adic Mellin pre-translate} with the $3$-adic Mellin transform
	\begin{equation}\label{equation: translated 3-adic Mellin}
	\overset{
	\begin{tikzpicture}
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	...
	\ \ =\  
	\underset{\QQ^\times_3}{\int}\!f\big(\tfrac{4}{5}x\big)\ \omega(x)\ |x|^{s}_3\ d^\times\!x.
	\end{equation}
\vskip -.2cm
\noindent
Because the product $\tfrac{4}{5}x$ appearing in this integral takes place in $\QQ^\times_3$, we first need to represent $\tfrac{4}{5}$ as an element in $\QQ^\times_3$. In fact, $4,\tfrac{1}{5}\in\ZZ_3$. We have $4=1+1\!\cdot\!3$, and we can use the fact that $5=2+1\!\cdot\!3$ to compute the representation of $\tfrac{1}{5}$ in $\ZZ_3$ as
	$$
	\frac{1}{5}
	\ \ =\ \ 
	2+
	{\color{blue}
	\underset{\text{repeats}}{\underbrace{
	{\color{black}
	0\!\cdot\!3+1\!\cdot\!3^2+2\!\cdot\!3^3+1\!\cdot\!3^4}
	}
	}
	}
	+\cdots+0\!\cdot\!3^{4n+1}+1\!\cdot\!3^{4n+2}\!\cdot\!3^{4n+3}+1\!\cdot\!3^{4n+4}+\cdots.
	$$
From this and the presentation $4=1+1\!\cdot\!3$ we obtain the presentation
	$$
	\frac{4}{5}
	\ \ =\ \ 
	2+
	...
	\underset{\text{repeats}}{\underbrace{
	{\color{black}
	2\!\cdot\!3+1\!\cdot\!3^2+0\!\cdot\!3^3+1\!\cdot\!3^4
	}
	}
	}
	}
	+\cdots+2\!\cdot\!3^{4n+1}+1\!\cdot\!3^{4n+2}+ 0\!\cdot\!3^{4n+3}+1\!\cdot\!3^{4n+4}+\cdots.
	$$
Truncating this last expression at $\text{cond}_3(f)$ lets us evaluate the integral in Equation \eqref{equation: translated 3-adic Mellin}.
\end{example}

\begin{trial}
{\bf Ramified \& unramified movement of three voices at different primes.}
\normalfont
\href{https://drive.google.com/file/d/15h0im6nR1iaH2bCNj41s0U5UlE_6i3fM/view?usp=sharing}{\ExternalLink{\em p-adic\underline{\ \ }GL1\underline{\ \ }trial\underline{\ \ }003.wav}}
The movement of the voices here is built over the tonics $110$, $110$, and $220$, respectively. {\color{red} [roughly]}
	$$
	f(x)\ =\ \sum_{n=1}^{3^6-1}n\cdot\mathbbm{1}_{[n]^{\le 5}+3^{6}\ZZ_3}(x)
	$$
	$$
	g(x)\ =\ \sum_{n=1}^{5^4-1}n\cdot\mathbbm{1}_{[n]^{\le 3}+5^{4}\ZZ_5}(x)
	$$
	$$
	h(x)\ =\ \sum_{n=1}^{7^4-1}n\cdot\mathbbm{1}_{[n]^{\le 3}+7^{4}\ZZ_7}(x)
	$$
Each voice in the three-part texture is essentially a chorale built from chords of $p$-adic Mellin transforms of the form
	$$
	\overset{
	\begin{tikzpicture}
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!f\!\circ\!\tfrac{1}{k}}(\omega, s)
	+
	\overset{
	\begin{tikzpicture}
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!g\!\circ\!\tfrac{1}{\ell}}(\omega, s)
	+
	\overset{
	\begin{tikzpicture}
	\draw[black, thick] (0,0) to [out=15, in=195] (.7,0);
	\end{tikzpicture}
	}{\!h\!\circ\!\tfrac{1}{m}}(\omega, s),
	\ \ \ \text{for certain}\ k,\ell,m\in\ZZ_{\ge1}.
	$$
\end{trial}

\begin{trial}
{\bf Unramified movement of a single voice superimposed with itself.}
\normalfont
\end{trial}

\begin{trial}
{\bf Ramified movement of a single voice superimposed with itself.}
\normalfont
\end{trial}

\begin{trial}
{\bf Ramified \& unramified movement of a voice and its $\pmb{\varepsilon}$ tonic deformation.}
\normalfont
\end{trial}
...
\begin{subsection}{On the structure of $\pmb{\bold{GL}_{1}(\mathbbm{A})}$-signals.}\label{subsection: structure of idelic signals}
In this \S\ref{subsection: structure of idelic signals}, we describe some of the structure hidden in $\bold{GL}_{1}(\mathbbm{A})$-signals that is especially relevant to musical considerations. We begin with an example, and then provide general statements.

\begin{example}
{\bf Structure of $\pmb{\bold{GL}_{1}(\mathbbm{A})}$-signals supported at 3 and 5.}
\normalfont
Suppose that we have a function $f:\bold{GL}_{1}(\A)\lra\CC$ supported at the primes $3$ and $5$. This means that $f$ can be written as a product
	$$
	f(x)
	\ =\ 
	f_3(x_3)\cdot f_5(x_5),
	\ \ \ \text{where}\ x_3\in\QQ_3\ \text{and}\ x_5\in\QQ_5.
	$$
Assume that the factors $f_3$ and $f_5$ are locally constant and compactly supported. Let $\omega:\bold{GL}_{1}(\A)\lra\CC^\times$ be any character obtained as a product
	$$
	\omega(x)\ =\ \omega_3(x_3)\cdot\omega_{5}(x_5)
	$$
of normalized unitary characters $\omega_3:\bold{GL}_{1}(\QQ_3)\lra\CC^\times$ and $\omega_5:\bold{GL}_{1}(\QQ_5)\lra\CC^\times$.
Then the ({\em adelic}) {\em Mellin transform} of $f$ at $\omega$ is the product of $p$-adic Mellin transforms
	$$
	\widetilde{f}(\omega, s)
	\ =\ 
	\overset{
	\begin{tikzpicture}
	\draw[black, thick] (0,0) to [out=35, in=215] (.35,0);
	\end{tikzpicture}
	}{\!\!f_{3}\!\!}(\omega_3, s)
	\cdot
	\overset{
	...
	$$
(See \cite[p. {\color{red} ???}\ \!\!]{GHv1} for details.) Suppose that $f_3$ and $f_5$ are supported on the subsets
	$$
	\bigsqcup_{m=1}^{M_{\text{max}}}3^{m}\ \ZZ_{3}^\times
	\ \ \subset\ \ \bold{GL}_{1}(\QQ_3)
	\ \ \ \ \ \ \ \ \ \text{and}\ \ \ \ \ \ \ \ \ 
	\bigsqcup_{n=1}^{N_{\text{max}}}5^{n}\ \ZZ_{5}^\times
	\ \ \subset\ \ \bold{GL}_{1}(\QQ_5),
	$$
respectively, for $M_\text{max},N_\text{max}>1$. Then the adelic Mellin transform $\widetilde{f}(\omega,i2\pi\lambda_0 t)$ of $f$ is of the form
	$$
	\Big(\sum_{m=1}^{M_\text{max}}c_m e^{i 2\pi\ \text{log}(3^m)\ \lambda_0 t}\Big)
	\cdot
	\Big(\sum_{n=1}^{N_\text{max}}c_n e^{i 2\pi\ \text{log}(5^n)\ \lambda_0 t}\Big)
	\ \ =\ \ 
	\sum_{m=1}^{M_\text{max}}\sum_{n=1}^{N_\text{max}}c_{mn}e^{i2\pi\ \!\big(\ \!m\ \!\text{log}(3)+n\ \!\text{log}(5)\ \!\big)\ \!\lambda_0 t}.
	$$
This factor $m\ \text{log}(3)+n\ \text{log}(5)$ of the frequency in the terms on the right-hand side of the above equation imply that the signal contains multiple independent overtone series (see Figure \ref{figure: overtone series in signal}).
	\begin{figure}[ht]
	\vskip -.3cm
	$$
	\includegraphics[scale=.05]{log.jpg}
	$$
	\vskip -.3cm
	\caption{{\bf Overtone series within a $\text{GL}\pmb{{}_{1}(\A)}$-signal supported at 3 and 5.} {\color{red} [...]} The diagonal {\color{gray} gray lines} in the figure are equi-frequency level curves, meaning that all signals along a given diagonal {\color{gray} gray line} have the same frequency.}
	\label{figure: overtone series in signal}
	\end{figure}
Indeed, {\color{red} [...]}

	Note also that we can ensure that the lowest tone sounded in the adelic Mellin transform $\widetilde{f}(\omega,\ \!i 2\pi\lambda_0t)$ has frequency
	$$
	\text{log}(3\!\cdot\!5)\ \lambda_0\ \ \text{Hz}.
	$$
Thus we can ensure that the lowest tone in $\widetilde{f}(\omega,\ \!i 2\pi\lambda_0t)$ is $\lambda_0$ by making the replacement
	$$
	\lambda_0
	\ \longmapsto\ 
	\frac{\lambda_0}{\text{log}(3\!\cdot\!5)}.
	$$
	
{\color{red} [Explain how all the sub-signals are interrelated, with a special focus on how adelic multiplication by $\ZZ$ affects the signals. A main goal here is to arrive at the observation that its really powers of primes that connect (loosely) to standard tonal movement...]}
...
\begin{subsection}{Perfect fifths, major thirds, and Galois actions.}
\ {\color{red} [All wrong. Need to fix... should be about equal temperament...]} Lots of important intervals arise from ``down-shifting overtones into the octave,'' i.e., as the octave translate of a given overtone that puts the resulting tone in the first octave over the root. Stated mathematically, if $n$ is a positive integer with corresponding overtone $n\lambda_0$ over the tonic $\lambda_0$, then there exists a unique positive integer $k$ such that
	$$
	{\color{blue}\underset{\text{tonic}}{\underbrace{{\color{black}\lambda_0}}}}
	\ \ \le\ \
	\frac{n}{2^k}\lambda_0
	\ \ <
	{\color{blue}\underset{\text{over tonic}}{\underset{\text{1 octave}}{\underbrace{{\color{black}2\lambda_0}}}}}.
	$$
The uniqueness of $k$ becomes clear when you see that the above condition is equivalent to the condition that $2^k\le n<2^{k+1}$. For example, a just-intoned perfect fifth is
	$$
	\frac{3}{2}\lambda_0,
	$$
a just-intoned major third is 
	$$
	\frac{5}{4}\lambda_0=\frac{5}{2^2}\lambda_0,
	$$
a just-intoned major second is
	$$
	\frac{9}{8}\lambda_0=\frac{9}{2^3}\lambda_0,
	$$
a just-intoned major seventh is
	$$
	\frac{15}{8}\lambda_0=\frac{15}{2^3}\lambda_0,
	$$
and a (just-intoned) harmonic seventh is
	$$
	\frac{7}{4}\lambda_0=\frac{7}{2^2}\lambda_0.
	$$
In fact, many of the familiar intervals that don't appear in this list arises by running s similar trick with undertones.

	In terms of standard audio signals {\color{red} [...]}
	$$
	f(\lambda_0t)\mapsto f\big(\tfrac{n}{2^k}\lambda_0t\big).
	$$
If we want to attempt to do the ``same'' thing with $p$-adic or adelic audio signals, we have to introduce either a local or global 
...
\normalfont
Fix a field $K$ and let $V$ be a $K$-vector space with $K$-linear dual 
	$$
	V^\ast\ :=\ \ \text{Hom}_{K\text{-{\bf Vec}}}(V,K).
	$$
Consider the tensor product $V^\ast\otimes_{K}V$. An element $\beta$ can be expressed as a tensor product is a sum
	$$
	\beta\ =\ \sum_{i=1}^{n}a_{i}\ \varphi_{i}\otimes v_{i}
	$$
for functionals $\varphi_i\in V^\ast$ and vectors $v_i\in V$. Note that this sum defines a function $\beta:V\lra V$ given by
	$$
	\beta(u)\ =\ \sum_{i=1}^{n}a_{i}\ \varphi_{i}(u)\ v_{i}
	$$
for each $u\in V$. We claim that this function is independent of the presentation of the element $\beta$ given by the sum {\color{red} [...Indeed...]}

If $V$ is finite-dimensional, say $d=\text{dim}_{K}d$, then we can fix a basis $\{e_i\}_{i=1}^{d}$ of $V$ and its dual basis $\{e^\ast_i\}_{i=1}^{d}$ of $V^\ast$, so that the sum above becomes unique:
	$$
	\sum_{1\le i,j\le d}\!\!a_{ij}\ e^\ast_i\otimes e_j.
	$$
\end{ssubsection}
