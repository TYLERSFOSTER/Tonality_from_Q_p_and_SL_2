...
\begin{algorithm}
\caption{$p$-Adic coordinates for non-negative real numbers}\label{alg:two}
\KwData{positive prime base $p\in\ZZ_{>0}$, non-negative real number $t\in\RR_{\ge0}$, positive integer $m\in\ZZ_{>0}$}
\KwResult{$p$-adic number $a\in\QQ_{p}$ assigned to $t$ at real resolution $p^{-m}$, and non-negative real error $\varepsilon\in\RR_{\ge0}$}
$r \gets$ {\bf copy of} $t$ \label{line: define r} \tcp*[r]{copy of our argument $t$, to modify over course of algorithm}
$n \gets 0$\; \label{line: define n}
\While{$p^{n} \le r$
}{
    $n \gets n+1$ \label{line: increase n by 1}  \tcp*[r]{\ref{line: define n}-\ref{line: increase n by 1} end with $n\in\ZZ_{\ge 0}$ such that  $p^{n-1}\le r<p^{n}$}
}
{\bf set} $L$ {\bf to empty list}
\; \label{line: define fractional list}
\For{$i\gets n-1$ \KwTo $-m$ \KwBy $-1$
}{
$a_{-i}=\lfloor\tfrac{r}{p^{i}}\rfloor$
\;
%\While{$(a_{-i}+1)\cdot p^i\le r$}{
%$a_{-i}\gets a_{-i}+1$ \;
%}
$r\gets r-a_{-i}p^{i}$ \;
{\bf append} $a_{-i}$ {\bf to list} $L$ \label{line: append fractional digit} \tcp*[r]{\ref{line: define fractional list}-\ref{line: append fractional digit} end with $L=(a_{-n},\dots,a_{-1},a_0,a_1,\dots,a_{m})$}
}
$\varepsilon\gets r$ 
\tcp*[r]{\ref{line: define fractional list}-\ref{line: positive part} end with $0\le \varepsilon<p^{m}$ such that $t=\varepsilon+\sum_{a_j\in L}a_{j}p^{-j}$}
\label{line: positive part}
$a \gets$ {\bf {\em p}-adic number} $p^{-m}\cdot\sum_{a_{j}\in L}a_{j}p^{j}$ \;
{\bf return} $a$, $\varepsilon$
\end{algorithm}
